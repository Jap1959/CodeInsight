...
SubmitProblem = async (data, UserName) => {
    ...
    const combinedDateTime = formattedDate.replace(/\//g, '') + formattedTime.replace(/:/g, '');
    ...
    if (res === true) {
        try {
            const Problem = await Schema.Problem.findOne({ ProblemName: data.Problem });
            ...
            SendQueue.sendCodeToCompileQueue(code, language, testCases, numericValue, id, data.Time, data.EndTime);
...
const Standings = async (UserName, ContestName) => {
    try {
        const standing = await Schema.Standing.findOne({ UserName: UserName, ContestName: ContestName });
